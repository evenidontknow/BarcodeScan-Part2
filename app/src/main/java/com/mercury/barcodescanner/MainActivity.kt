...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        barcodeViewModel = ViewModelProviders.of(this)
        ...
        isData = intent.getBooleanExtra("fromScannerActivity",
            false)
        getContactsData()
        if(isData){
            processData()
    ...
    private fun processData() {
        barcodeViewModel.isAlreadyPresentInFirebase(
            intent.getStringExtra("email") ?: "NA").observe(this,
            Observer { isPresent->
                Log.d("Data", isPresent.toString())
                if (isPresent == 0){
                    addDataToDatabases()
                }else if(isPresent == -1){
                    Toast.makeText(this,"Contact email already present in database",
    ...
    private fun addDataToDatabases() {
        ...
        barcodeViewModel.getSingleContact(email = contact.email)
            .observe(this, Observer {roomContact ->
            roomContact?.let {
                barcodeViewModel.getSingleContact(email = contact.email)
    ...
    private fun getContactsData() {
        barcodeViewModel.getDataFromFirebase()
            .observe(this, Observer {contacts ->
