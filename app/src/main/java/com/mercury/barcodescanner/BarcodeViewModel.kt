...
class BarcodeViewModel(application: Application)
    ...
    private var contactDatabase: ContactDatabase =
        ContactDatabase.getInstance(context = this.getApplication())
    ...
    fun insertContact(contact: Contact) = viewModelScope
    ...
    fun getSingleContact(email: String) : LiveData<Contact> {
        singleContact = contactDatabase
            .contactDAO().getContact(email = email)
